...
public class SendSchedule {
    ...
    public void sendWeekMsg(){
        ...
        try{
            pushMessageService.pushMessage(TaoMiniConstant.TEMPLETEIDWEEK);
        }catch (Exception e){
            LOGGER.error("发送失败:{}", e.getMessage());
            LOGGER.error("失败", e);
        }
    ...
    public void sendMonth(){
        ...
        if (c.get(Calendar.DATE) == c.getActualMaximum(Calendar.DATE)) {
            try{
                pushMessageService.pushMessage(TaoMiniConstant.TEMPLETEIDMONTH);
            }catch (Exception e){
                LOGGER.error("发送失败:{}", e.getMessage());
                LOGGER.error("失败", e);
            }
    ...
    public void sendXigu(){
        ...
        try{
            ...
            pushMessageService.pushMessage(TaoMiniConstant.TEMPXIGUAQUCHONG);
        }catch (Exception e){
            LOGGER.error("发送失败:{}", e.getMessage());
            LOGGER.error("失败", e);
        }
